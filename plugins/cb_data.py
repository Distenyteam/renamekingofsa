...
async def doc(bot,update):
     ...
     c_time = time.time()
     try:
     	path = await bot.download_media(message = file, progress=progress_for_pyrogram,progress_args=( "@FilmPiratesGroup",  ms, c_time   ))
     ...
     if thumb:
     		...
     		c_time = time.time()
     		try:
     			await bot.send_document(update.message.chat.id,document = file_path,thumb=ph_path,caption = f"{new_filename}",progress=progress_for_pyrogram,progress_args=( "@FilmPiratesGroup",  ms, c_time   ))
     			await ms.delete()
     			os.remove(file_path)
     			os.remove(ph_path)
     		except Exception as e:
     		...
     		c_time = time.time()
     		try:
     			await bot.send_document(update.message.chat.id,document = file_path,caption = f"{new_filename}",progress=progress_for_pyrogram,progress_args=( "@FilmPiratesGroup",  ms, c_time   ))
     			await ms.delete()
     			os.remove(file_path)
     		except Exception as e:
     			await ms.edit(e)
